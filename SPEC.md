...
- ~~**MCP 서버**~~ → 마일스톤 9에서 만들었다. `mcp/`의 별도 Worker `invest-mcp`, 읽기 툴 6개, OAuth는 직접 구현(§7.8). 마일스톤 10에서 쓰기 툴(보유 가져오기·의견 기록·`add_note`)을 더했지만 **초안까지만** 쓴다(§7.9).
...
### 5.2 투자의견
#### 5.2.1 폼
필수: 목표가, 등급. 선택: **결론**, 핵심 논리, 리스크, 밸류에이션 근거(§5.2.7), 목표 시점(기본 12개월).

- **결론** [확정: 2026-09-22]: "예상 EPS 얼마에 멀티플 몇 배를 왜 줘서 목표주가 얼마, 그래서 무슨 의견"을 한 덩어리 글로 적는 칸(`views.conclusion`, `0013`). 숫자는 `target_price`·`valuation`에 있고 여기는 그 **이유**를 남긴다. 기록을 펼치면 결론 → 핵심 논리 → 리스크 순으로 보인다. **결론은 항상 문단으로** 렌더한다(한 덩어리 산문이라 불릿을 붙이지 않는다). 핵심 논리·리스크는 아래 규칙을 따른다.
- **임시 저장** [확정: 2026-09-22]: 새 의견을 쓰는 동안 입력이 바뀔 때마다 기기(localStorage)에 임시 저장한다. 시트를 닫았다 열면 "쓰다 만 내용을 불러왔습니다"와 함께 복원되고, 저장에 성공하면 지워진다. 내용이 있으면 **시트 바깥을 눌러도 닫히지 않는다**(닫으려면 × 또는 Esc). 서버로는 보내지 않는다.

핵심 논리·리스크 본문은 **쓴 모양을 그대로 살린다** [확정: 2026-09-22]. 빈 줄로 나누면 문단(문단 안 줄바꿈도 유지), 한 줄씩 쓰면 불릿이다. 길이 제한은 없고 길게 쓰면 문단 하나가 길어질 뿐이다. 판정은 `format.js`의 `textBlocks`가 한다.
...
- 화면: **적중률** 크게, 옆에 **평균 목표수익률 · 평균 실제수익률 · MAE(평균 abs_error)** 작게, 표본 수 n 항상 표시. horizon 미경과는 "진행 중"으로 따로 셈. 기준 설명은 블록 제목 오른쪽 `?`에 둔다.
- 소급 기록(§5.2.6)도 **같은 숫자에 함께 센다** [확정: 2026-09-22]. 중요한 판단만 소급해 넣기로 해 나눌 실익이 없었다. `views.backdated`는 계속 쌓이므로 다시 가르려면 `performance()`만 고치면 된다.
- 평균 목표수익률 − 평균 실제수익률 = 편향(낙관/비관). 별도 계산 없이 두 숫자로 읽힌다.
- 시장 대비 초과수익 컬럼은 2차 [미정].
- 평가 채움은 크론(미국장 일일). 두 단계다 [확정: 2026-09-23]:
  - **조기 적중**: 목표 기간 중이라도 목표가에 닿으면(`high ≥ target`/`low ≤ target`) 그날로 `hit=1`, `hit_date`를 찍는다. 적중 기준이 "한 번이라도 도달"이라 닿는 순간 결과가 확정된다. 적중률에 **바로** 들어간다.
  - **만기 평가**: `created_at + horizon_months`가 지나면 만기 종가로 `actual_return`·`abs_error`를 채우고 `evaluated_at`을 찍는다.
  - 그래서 적중률 표본(만기 + 조기 적중)과 수익률·MAE 표본(만기만)이 다를 수 있고, 화면에 두 건수를 모두 적는다("판정 N건 · 기간 중 도달 K건 포함", "수익률·MAE는 목표 기간이 끝난 M건 기준").
  - 만기 전에 편집으로 목표가를 바꾸면 조기 적중을 지우고 다음 크론이 새 목표가로 다시 본다.
- **MAE**는 Mean Absolute Error(평균 절대 오차) — 목표수익률과 실제수익률 차이를 방향 없이 평균낸 값. 화면에는 `MAE 평균 오차`로 표기한다.

#### 5.2.6 과거 날짜로 소급 기록 [확정: 2026-09-22]
앱을 만들기 전에 실제로 했던 판단을 넣을 수 있다. 폼의 **과거 날짜로 기록**을 켜고 날짜를 고르면 `POST /views`에 `as_of`가 실린다.

- **스냅샷은 그 날짜로 복원한다.** `price_at`은 그날(휴장이면 직전 거래일) 종가, `price_at_source`는 `close YYYY-MM-DD`. 컨센서스는 `estimates.as_of <= 날짜`로 잘라 그 시점까지 알려진 값만, `per_at`도 그 시점까지 끝난 4개 분기로 계산한다. 나중에 받은 값이 과거 기록에 새어 들어가지 않는다.
- **`created_at`은 그 날짜 00:00 KST.** 사후평가 크론의 horizon 계산도 이 날짜 기준이므로, 만기가 이미 지난 소급 의견은 다음 크론에서 곧바로 평가된다(가격은 사실이므로 정직한 채점이다).
- **`views.backdated = 1`로 남긴다** (`0012`). 2026-09-22부터 화면에는 배지를 띄우지 않고 성과도 함께 집계한다(§5.2.5). 구분이 필요하면 MCP `get_investment_views`의 `backdated` 필드로 본다.
- **시세가 있는 날짜만.** 그 종목 봉이 해당 날짜로부터 7일 이내에 없으면 400으로 거절한다(상승여력을 얼릴 수 없으므로). 미래 날짜도 거절, 오늘 날짜는 평소 기록과 같다.
- **입력 경로는 앱 폼뿐이다.** MCP `add_investment_view`는 지금도 '제출 시각'만 쓴다. 대화하는 모델이 날짜를 지어낼 여지를 두지 않는다.

#### 5.2.7 밸류에이션으로 목표가 만들기 [확정: 2026-09-22]
목표가를 감으로 적는 대신 **기준값 × 배수**로 만들 수 있다. 폼의 "밸류에이션으로 목표가 계산"을 켜면 지표(PER·PBR·PSR) · 기준값 · 배수를 받아 목표가 칸을 채운다.

- **계산은 코드가 한다.** 곱셈은 폼에서, 밴드 쪽 시나리오 환산은 `impliedTarget`이 한다. LLM은 관여하지 않는다.
- **기준값 제안**: 앱이 아는 최신 TTM 값(EPS·BPS·주당매출)을 버튼 하나로 넣는다. 재무 통화가 주가 통화와 다르거나 ADR이면(밴드 화면과 같은 기준) 제안하지 않고 직접 넣게 한다. 미래 추정치를 쓰고 싶으면 덮어쓰면 된다.
- **계산값은 출발점일 뿐** 목표가를 직접 고쳐도 된다. 고친 값이 저장된다.
- **가정은 기록에 남는다.** `views.valuation`에 `{metric, value, multiple, implied_target}`으로 저장하고, 펼친 의견의 **목표가 산출 방식**에 `EPS $15.00 × PER 40배 = $600.00`처럼 보인다. 기준값에는 통화, 배수에는 `배`를 붙인다(`basisText`). EV/EBITDA는 순부채·주식수를 거치므로 곱셈 결과를 주당 목표가로 쓰지 않고 앞부분만 적는다. 밴드 화면(§5.4)에서 넘어온 가정도 같은 칸·같은 표기를 쓴다.
- **편집에서도 고칠 수 있다** [확정: 2026-09-22]. 편집 시트에 같은 블록이 기존 값으로 채워져 열리고, 배수를 바꾸면 목표가와 산출 방식이 함께 갱신된다. 목표가만 바꿔 근거와 어긋나는 일을 막는다.
- **MCP**도 `target_basis`로 같이 준다. 대화에서 "그때 무슨 배수를 가정했지?"에 답할 수 있다.
- EV/EBITDA는 주식수·순부채가 필요해 폼에서는 빼고 밴드 화면 경로를 쓴다(서버 `impliedTarget`은 네 지표 다 지원).

### 5.9 종합 의견 (테마·섹터 메모) [확정: 2026-09-22]
화면 이름은 **종합 의견**이다(2026-09-23 개명: 탭·섹션·버튼·시트·홈 블록·종목 상세 '관련 종합 의견'·가져오기 대기). 코드·DB·MCP 툴 이름(`notes`, `get_notes`, `add_note`)은 그대로 두고 MCP 설명 문구만 바꿨다.

종목이 아니라 **테마·섹터·매크로**에 대한 생각을 적는 곳이다(예: "에이전틱 AI 확산으로 CPU 주목"). 종목 의견(§5.2)과 **일부러 다른 물건**이다.

- **정량 평가를 흉내 내지 않는다.** 목표가·기간·등급이 없고 기계가 채점하지 않는다. `views`의 적중률·MAE에 들어가지 않는다. 억지로 섞으면 채점할 수 없는 글이 성과 평가를 오염시킨다.
- **칸**: 제목, 본문(자유), 태그(JSON 배열), 방향성(`positive|neutral|negative`, 비워도 됨), 연결 종목(`note_securities`), `created_at`·`backdated`, `edited_at`. 테이블은 `notes`(`0014`).
- **분류는 고정하지 않는다.** 매크로/섹터/테마 같은 고정 종류를 만들지 않고 **자유 태그**로 시작한다. 쓰면서 쌓인 태그를 폼에서 추천한다(많이 쓴 순). 패턴이 보이면 그때 승격한다 [미정].
- **방향성은 등급이 아니다.** 8등급(§5.2.2)과 다른 가벼운 표시이며 성과 평가와 무관하다.
- **과거 날짜로 기록 가능**(§5.2.6과 같은 방식). 메모는 가격 스냅샷을 얼리지 않으므로 날짜 제약이 없다. 미래 날짜만 막는다.
- **회고는 사실만.** 메모를 펼치면 연결 종목의 `기록일 종가 → 지금 가격 → 변화율`을 보여준다. 적중·실패 판정은 붙이지 않고, 화면에도 "사실만 적는다"고 쓴다.
- **어디에 보이나**: 투자의견 탭의 `종목 의견 / 테마·메모` 전환, 종목 상세의 `관련 메모`, 홈의 `테마·메모`(접힘).
- **MCP**: `get_notes`(읽기), `add_note`(제안 → 초안 대기열, §7.9). 소급 날짜는 앱에서만 — 대화에서는 제출 시각으로 들어간다.

#### 5.2.8 의견 목록 표기 [확정: 2026-09-22]
- 앞면은 **두 줄 압축**(A안, 2026-09-23): 왼쪽에 종목·등급과 그 바로 아래 날짜(간격 2px), 가운데 열에 **목표가**(크게), 오른쪽에 `분석 당시 상승 여력 / 분석 이후`를 라벨·숫자 쌍으로 두 줄 쌓는다. 폰 폭(≤480px)에서는 라벨을 `분석 당시`로 줄인다. 목표에 닿았으면 날짜 옆에 `목표 도달`.
- 날짜는 올해면 `8/26`, 다른 해면 `2025/8/26`. 시:분은 앞면에 쓰지 않는다(`dayStamp`).
- 펼친 뒤는 **2열**(2026-09-23): 왼쪽 `분석 당시 주가`·`목표 기간`, 오른쪽 `목표가 산출 방식`·`목표 기간 진행률`. 그때 컨센 목표가는 셋째 줄 왼쪽. 산출 방식은 색 없이 **숫자(기준값·배수·목표가)만 굵게**(`basisParts`). 이 영역의 전체 날짜는 `2027/08/26` 형식(`slashDate`)으로 쓴다(투자의견 화면만).
- 옛 이름 대응: 기록 시 → 분석 당시 상승 여력, 이후 → 분석 이후, 기록 시점 가격 → 분석 당시 주가, 목표 시점 → 목표 기간, 상태 → 목표 기간 진행률, 목표가 근거 → 목표가 산출 방식.
- 행의 버튼은 **업데이트**(그 종목 새 의견, 직전 내용으로 채워서 열림) · **편집**(그 행 덮어쓰기) · **삭제**(확인 창).
...
소급 기록에서 정한 것 (2026-09-22):
- 예측 기록의 값어치는 "그때는 몰랐다"에서 나온다. 그래서 소급 입력을 막는 대신 **표시하고 분리 집계**하기로 했다. 막으면 앱 이전의 판단이 통째로 사라지고, 섞으면 적중률이 사후에 좋아진다.
- 가격·PER·컨센서스는 D1에 이미 있는 과거 데이터로 복원하므로 스냅샷 자체는 사실이다. 사후인 것은 판단뿐이고, 그것을 `backdated` 한 칸으로 구분한다.
- 입력은 앱 폼에서만 받는다. MCP까지 열면 대화 중에 날짜가 만들어질 수 있다.
- 날짜 범위는 그 종목 시세가 있는 날까지. 가격을 모르면 상승여력을 얼릴 수 없어 기록의 의미가 없다.

테마·메모에서 정한 것 (2026-09-22):
- 종목 의견 테이블에 끼워 넣지 않고 `notes`로 분리했다. 목표가·등급을 필수로 만들면 쓸 수 없는 글이 많고, 채점 못 하는 기록이 적중률에 섞인다.
- 고정 분류(매크로/섹터/테마) 대신 자유 태그로 시작한다. 실제로 뭘 쓰는지 보고 나중에 정한다.
- 방향성 3단계(긍정·중립·부정)만 둔다. 나중에 "내가 긍정적으로 봤던 테마들"을 모아보기 위한 최소한의 구조다.
- 연결 종목의 이후 수익률은 **사실로만** 보여준다. 숫자는 보여주되 적중·실패 판정은 하지 않는다.
