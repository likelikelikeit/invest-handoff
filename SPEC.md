...
- **결론** [확정: 2026-09-22]: "예상 EPS 얼마에 멀티플 몇 배를 왜 줘서 목표주가 얼마, 그래서 무슨 의견"을 한 덩어리 글로 적는 칸(`views.conclusion`, `0013`). 숫자는 `target_price`·`valuation`에 있고 여기는 그 **이유**를 남긴다. 기록을 펼치면 결론 → 핵심 논리 → 리스크 순으로 보인다.
...
- **`views.backdated = 1`로 표시하고 성과는 따로 센다** (`0012`). 화면에는 `사후 입력` 배지가 붙고, 성과 평가 블록은 "사전 예측만"으로 집계하며 소급분은 아래 줄에 건수와 적중률을 따로 적는다. MCP `get_investment_views`도 `backdated` 필드와 `backdated_summary`를 나눠서 준다.
...
- **가정은 기록에 남는다.** `views.valuation`에 `{metric, value, multiple, implied_target}`으로 저장하고, 펼친 의견에 `EPS 22,358 × PER 14 = 313,012원` 한 줄로 보인다. 밴드 화면(§5.4)에서 넘어온 가정도 같은 칸·같은 표기를 쓴다.
