...
async function sec(env, id) {
  const row = await env.DB.prepare("SELECT id, name, ticker, ysym, market, currency, dart_corp_code FROM securities WHERE id = ?1").bind(id).first();
...
export async function getFundamentals(request, env, headers, p) {
  ...
  const [financials, estimates, events] = await env.DB.batch([
    env.DB.prepare(
      "SELECT period_end, period_type, revenue, operating_income, net_income, eps, bps, ebitda, net_debt, shares_out, source, fetched_at " +
  ...
  return json({ ok: true, security: s, financials: financials.results, estimates: estimates.results, next_earnings: events.results[0] || null }, 200, headers);
}
